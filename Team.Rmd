...
# Part One
### 1.
Here we've successfully made the player and opponenent name columns into factors.
```{r}
# offense
...
### 2.
Here we're able to make all statistics in `offense`, `defense`, and `biography` numerical (instead of characters).
```{r}
#This is so we can separate the Passing_CMP-ATT column in offense
...
### 3.
Here we separated `Height` into `Feet` and `Inches` where we used those columns and calculated a total height value which we replaced with `Height` (which is now in total inches).
```{r}
# separating into ft and in
biography <- biography %>%
  separate(Height, c('Feet', 'Inches'))
# calculating into a total inches column, then removing the extra columns used
biography <- biography %>%
...
goodOffense <- offClean %>% 
  filter(stat == "Receiving_YDS" & (Opponent_Opponent == "Oklahoma" | Opponent_Opponent == "Oregon"))
# creating scatter plot and boxplot
ggplot(goodOffense, aes(x = Amount, y = factor(Opponent_Opponent))) + geom_point() +
  ggtitle("Best ISU Offensive Game") + ylab("Opponent") + xlab("Receiving Yards")
...
defensivePlayers$State <- factor(defensivePlayers$State)
offensivePlayers$State <- factor(offensivePlayers$State)

# creating bar chart for offensive and defensive data
ggplot(offensivePlayers, aes(x = State)) + geom_bar() + 
  ggtitle('ISU Offense Player Home States') + xlab('State') + ylab('Count')
ggplot(defensivePlayers, aes(x = State)) + geom_bar() + 
